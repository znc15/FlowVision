...
function ClassNode({ id, data, selected }: NodeProps<FlowNode>) {
  ...
  return (
    <div
      className={`w-56 bg-surface-container-lowest rounded-xl ghost-border-soft overflow-hidden relative transition-all duration-200 ${
    ...
    >
      ...
      <div className="p-3 border-b border-outline-variant bg-surface-container-low/50">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-bold text-blue-600 uppercase tracking-tighter ml-1">Class</span>
          <button
        ...
        <div className="text-center">
          {data.stereotype && (
            <div className="text-[10px] text-on-surface-variant mb-0.5">«{data.stereotype}»</div>
          )}
          {isEditing ? (
            <input
              autoFocus
      ...
      <div className="p-3 border-b border-outline-variant min-h-[40px]">
        {data.attributes && data.attributes.length > 0 ? (
          ...
          <span className="text-[10px] text-on-surface-variant/50 italic ml-1">No attributes</span>
        )}
      </div>
