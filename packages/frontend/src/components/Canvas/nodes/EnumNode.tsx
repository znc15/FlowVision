...
function EnumNode({ id, data, selected }: NodeProps<FlowNode>) {
    ...
    >
      ...
      <div className="p-3 border-b border-outline-variant bg-surface-container-low/50">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[10px] font-bold text-purple-600 uppercase tracking-tighter ml-1">ENUM</span>
          <button
            ...
            title="编辑节点"
          >
            <span className="material-symbols-outlined text-sm text-slate-400 hover:text-purple-500">more_horiz</span>
          </button>
        </div>
        <div className="text-center">
          <div className="text-[10px] text-on-surface-variant mb-0.5">«enum»</div>
          {isEditing ? (
            <input
              autoFocus
              value={editLabel}
      ...
      <div className="p-3 min-h-[40px]">
        {data.attributes && data.attributes.length > 0 ? (
          ...
          <span className="text-[10px] text-on-surface-variant/50 italic ml-1">No values</span>
        )}
      </div>

      <Handle type="target" position={Position.Top} id="top" className="w-3 h-3 !bg-purple-500" />
      <Handle type="source" position={Position.Bottom} id="bottom" className="w-3 h-3 !bg-purple-500" />
